...
fn classify_managed_file(existing: Option<&str>, desired: &str, body: &str) -> ManagedFileState {
    ...
    if existing.contains(MANAGED_FILE_MARKER) || existing.trim_end() == body.trim_end() {
...
pub fn write_managed_rules_file(path: &Path, body: &str) -> Result<bool> {
    ...
    let desired = managed_rules_contents(body);
    let existing = path
        .exists()
    ...
    if state == ManagedFileState::Unowned {
        ...
        eprintln!(
            "  This path is tokensave's own rules file and is rewritten on install and upgrade.\n  \
             Keep your own notes elsewhere, or remove tokensave's rules with \
             `tokensave uninstall`."
        );
    } else {
        crate::agent_note!(
...
pub fn check_managed_rules_file(dc: &mut DoctorCounters, path: &Path, agent_id: &str) {
    ...
    // Both spellings are up to date: with the provenance marker (#584), and
    // the bare body a pre-#584 install wrote. The latter gains its marker on
    // the next install, which is not drift the user needs to act on.
    let desired = managed_rules_contents(&expected);
    if installed.trim_end() == desired.trim_end() || installed.trim_end() == expected.trim_end() {
        dc.pass(&format!(
